...
## 2026-07-24 連淨／統一生機苦茶油：中國苦茶籽原料僅限官方具名回收批次

- 使用者要求查的是具名品牌／產品，不是泛用的「苦茶油可能中國來源」提醒；因此建立兩張 product-and-lot-specific `personal` 卡片：`aconpure-organic-camellia-oil-250ml-china-seed-batch-26v224xw01-personal`（連淨有機苦茶油 250mL，批號 `26V224XW01`）及 `uni-organic-first-press-camellia-oil-250ml-china-seed-batch-26s624xw01-personal`（統一生機初榨苦茶油 250mL，批號 `26S624XW01`）。
- 新北市衛生局 2026-07-22 公告可直接對應兩款產品、批號與販售通路；其 2026-07-23 直接公告明確稱**本次連淨苦茶籽原料由中國輸入**，並指出問題批次已流入統一生機等通路。中央社報導僅作交叉轉述，records 改以衛生局一手頁為主。
- 事實邊界：這是「中國苦茶籽原料／特定回收批次」，**不是中國製成品**。不得延伸為連淨或統一生機全品牌、其他容量、其他批號、其他年份均使用中國原料，也不能由苯駢芘事件推定原料必然違法或不安全。
- 確認目前台灣 PChome 個別連淨商品頁會標示台灣原產，但沒有逐批號橋接能力；不以該通路標示覆蓋或否定新北衛生局對本次批次原料來源的說明。
- 入庫後全量 validator PASS（210 records、10 checks、warnings/errors = 0）；目標驗證以兩款完整品名和兩個批號正查，並做兩筆產品之間的 `--not-expect` 交叉排除，均 PASS。
